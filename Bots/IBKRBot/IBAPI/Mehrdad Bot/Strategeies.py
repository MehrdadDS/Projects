...
class TradingStrategies:
    ...
    def short_wavy_tunnel_conservative_strategy(self, time_frame):
        """
        Checks if the latest closing price is above the specified EMA filters
        and returns True if all conditions are met.
    ...
    def investment_strategy(self,time_frame):
        db = self.data.copy(deep=True)
        trade_trigger = 'No'
        ...
        if (latest_close >= ema_12_filter and
            ema_12_filter >= wave_ema_high and
            ema_12_filter >= tunnel_ema_high and
            wave_ema_high >= tunnel_ema_low and
            latest_close  >= tunnel_ema_high) and ~(latest_close > tunnel_ema_low*1.3) :
