...
db = pd.read_excel(os.path.join(folder_path,'forecast_result_after_redistribution.xlsx'))
volumes_db = pd.read_excel(os.path.join(edd_folder_path,'ForecastResults.xlsx'),sheet_name="Forecast")
...
volumes = volumes_db.query(f" `Week` in {peak_weeks}")
total_eid_volume = df.groupby(['Year','Customer'])['Pieces'].sum().reset_index()
df = df.merge(total_eid_volume,on=['Year','Customer'],how='left')
df['terminal distribution'] = df['Pieces_x']/df['Pieces_y']
df['terminal distribution'] = df['terminal distribution'].fillna(0)


total_volumes = volumes.groupby(['Year','Customer'])['Pieces'].sum().reset_index()

df = df.merge(total_volumes,on=['Year','Customer'],how='left',)
...
df['Pieces'] = 0.99*df['Pieces']

df.to_excel(os.path.join(folder_path,'coverting_edd_volumes_to_eid.xlsx'),index=False)